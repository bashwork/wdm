...
import org.wdm.core.support.{MultipleSupport, SingleSupport}
import org.wdm.core.{ItemSet, Transaction, FrequentSet}
import scala.tools.nsc.Interpreter._

class MsGeneralizedSequentialPatternSpec extends FlatSpec
	with ShouldMatchers with PrivateMethodTester {

	behavior of "the MsGeneralizedSequentialPattern algorithm"
	...
	it should "generate candidate2 correctly" in {
		...
		val gsp = new MsGeneralizedSequentialPattern(sequence, support)
	...
	it should "generate frequent2 correctly" in {
		...
		val gsp = new MsGeneralizedSequentialPattern(sequence, SingleSupport[String](0.3))
	...
	it should "generate candidateN correctly" in {
		...
		val sequence = (1 to 5).map { x => Transaction(ItemSet(x.toString)) }.toList
		val gsp = new MsGeneralizedSequentialPattern(sequence, SingleSupport[String](0.1))
	...
	it should "generate frequentN correctly" in {
		...
		val gsp = new MsGeneralizedSequentialPattern(sequence, SingleSupport[String](0.25))
